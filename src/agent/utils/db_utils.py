...
class MySQLDatabaseManager:
    ...
    def __init__(self, connection_string):
        ...
        Args:
            connection_string:MySQK连接字符串，格式为：
                    mysql+pymysql://username:password@host:port/databasename
        """
    ...
    def get_tables_with_comments(self)->list[dict]:
        ...
        """
        try:
            #构建检查语句，从INFOMATION_SCHMA.TABLES中获取表名和注释
            query = text("""
            ...
            log.exception(e)
            raise ValueError(f"获取表名及名描述信息失败{str(e)}")
    def get_table_schema(self,table_names:Optional[List[str]] = None)->str:
        ...
        try:
            ...
            for table_name in tables_to_process:
                ...
                #使用get_pk_constraint 替代已弃用的get_primary_keys
                ...
                if indexes:
                    table_schema +="索引信息：\n"
                    for idx in indexes:
                        if not idx['name'].startswith('sqlite'):
                            table_schema +=f"  -{idx['name']}:{idx['column_names']} ({'唯一'if idx['unique'] else ''})\n"
    ...
    #         return "警告：建议使用SELECT或WITH查询，其他操作可能会被限制"
    #
    #     #尝试解析查询（不实际执行）
    #     try:
    #         with self.engine.connect() as connection:
    ...
    def validate_query(self,query:str)->str:
        ...
        try:
            with self.engine.connect() as connection:
                #根据数据库方言构建EXPLAIN查询
                #这里以MySQL为例，其他数据库请参考EXPLAIN语法
                if self.engine.dialect.name == 'mysql':
                    explain_query = text(f"EXPLAIN {query}")
                else:
                    #对于其他数据库（如PostgreSQL,SQLite）,他们的EXPLAIN语法类似
                    explain_query = text(f"EXPLAIN {query}")
                #执行explain如果查询无效，此处抛出异常
